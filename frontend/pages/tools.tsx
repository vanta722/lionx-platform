...
export default function Tools() {
  ...
  async function runQuery() {
    ...
    try {
      ...
      setResult(data)
      setRunState('done')
      setHistory(h => [{ tool: tool.name, input: input.slice(0,22)+'...', cost: tool.cost, time: 'Just now' }, ...h.slice(0,9)])
    } catch (e: any) {
      setRunState('error')
    ...
    <>
      ...
      <div className="md:hidden" style={{ background: '#050508', minHeight: '100vh', paddingBottom: 80 }}>
        ...
        <div className="px-4 pt-4">
          ...
          {history.length > 0 && (
            <div className="mt-4">
              <button onClick={() => setShowHistory(h => !h)} className="flex items-center justify-between w-full px-4 py-3 rounded-xl mb-2" style={{ background: '#0a0a16', border: '1px solid rgba(255,255,255,0.05)', color: '#7a8a9a', cursor:'pointer', fontFamily:'inherit' }}>
                <span className="text-xs font-bold uppercase tracking-wider">Recent Queries ({history.length})</span>
                <span>{showHistory ? '▲' : '▼'}</span>
              </button>
              {showHistory && history.map((h,i) => (
                <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl mb-1" style={{ background: '#0a0a16', border: '1px solid rgba(255,255,255,0.04)' }}>
                  <div>
                    <div className="text-xs font-bold" style={{ color: '#14b8a6' }}>{h.tool}</div>
                    <div className="text-xs font-mono" style={{ color: '#4a5a6a' }}>{h.input}</div>
                  </div>
                  <div className="text-xs font-bold" style={{ color: '#f5a623' }}>{h.cost} 🔥</div>
                </div>
              ))}
            </div>
          )}
